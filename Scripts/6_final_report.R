...
data_complete <- read_csv("Data/course_completion_by_user_course_or_department_report_KON.csv",
                          col_names = c("name", "emp_id","username", "email", "course", "cost.centre.name", "cost.centre", 
...
status_cancelled <- read_csv(file = "Data/emr___face_to_face_sessions_by_user_report_20160513.csv", 
...
data_complete %<>% anti_join(status_cancelled, c("username", "course")) 
...
div_not_prof_dist <- distinct(data_complete, email) %>% 
...
write_csv(div_not_prof, "/Training Reports/Reports/Final Reports/div_not_prof.csv", col_names = TRUE)
write_csv(div_not_prof_dist, "/Training Reports/Reports/Final Reports/div_not_prof_distinct.csv", col_names = TRUE)
